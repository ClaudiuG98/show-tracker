...
import { configureBackups } from "../backup/automatic-store";
import { AutomaticBackups } from "./components/AutomaticBackups";
...
function Layout({ children, status }: { children: React.ReactNode; status: string }) {
  return <><a className="skip-link" href="#main">Skip to content</a><header><a className="brand" href="#/watch-list"><img src="/icons/icon-32.png" alt="" aria-hidden="true"/> Show Tracker</a><nav aria-label="Primary">
...
export function SettingsPage({ tracker }: { tracker: Tracker }) {
  ...
  const syncFailure = tracker.local?.lastSyncFailure;
  const syncFailureDate = syncFailure ? new Date(syncFailure.failedAt) : undefined;
  ...
  /**
   * Plays the toolbar alert on demand. It is otherwise only reachable by waiting for an episode
   * to air while the browser is closed, which is not something anyone can sit down and check.
   *
  ...
  return <><PageHeading title="Settings" description="Automatic updates, backups, and extension information."/><section className="settings"><p className="eyebrow">TV information</p><h2>Updates</h2><p>Show and episode information is checked automatically once a day. Newly tracked shows are downloaded immediately.</p>
    <dl className="update-status" aria-label="Metadata update status">
      <div><dt>Automatic checks</dt><dd>Once a day</dd></div>
      <div><dt>Last successful check</dt><dd>{hasValidLastSync ? <time dateTime={lastSyncAt}>{formatInTimeZone(lastSyncDate, tracker.local!.settings.timezone, "PP · p")}</time> : "Not checked yet"}</dd></div>
...
<div className="show-actions"><AsyncButton busy={tracker.metadataAction === "refresh"} disabled={tracker.metadataAction !== undefined} busyLabel="Checking for updates…" successLabel="Updates checked" onAction={() => tracker.refreshMetadata()}>Check for updates</AsyncButton></div>{tracker.metadataError && <p className="error" role="alert">{tracker.metadataError}</p>}<p className="settings-help">Manual checks are optional. Use this when you want TVMaze changes before the next automatic check.</p><details className="settings-troubleshooting"><summary>Troubleshooting</summary><p>Re-download all metadata only to repair missing or incorrect show information. It clears the provider request cache and leaves your library and watch progress untouched.</p><AsyncButton busy={tracker.metadataAction === "redownload"} disabled={tracker.metadataAction !== undefined} busyLabel={`Downloading ${showCount} shows…`} onAction={redownloadMetadata}>Re-download all metadata</AsyncButton></details></section><section className="settings"><p className="eyebrow">TV information</p><h2>New episode alerts</h2><p>When an episode of a show you are watching becomes available, the extension icon is marked in red and flashes briefly. Hovering the icon lists the episodes. Nothing is sent to your operating system, so there are no desktop notifications to allow or block.</p><div className="pin-callout"><span className="pin-callout-mark" aria-hidden="true">📌</span><div><strong>Pin the extension to see these alerts.</strong><p>{import.meta.env.FIREFOX ? "Open Firefox’s extensions menu (the puzzle piece beside the address bar), open the gear menu beside Show Tracker, and choose Pin to Toolbar." : "Open Chrome’s extensions menu (the puzzle piece beside the address bar), find Show Tracker, and click the pin."} An unpinned extension is hidden inside that menu, so its icon — and any alert on it — is out of sight.</p></div></div><label className="checkbox-row"><input type="checkbox" checked={tracker.local?.settings.notifications ?? true} onChange={(event) => void toggleNotifications(event.target.checked)}/>Mark the icon when new episodes are available</label><div className="show-actions"><AsyncButton busyLabel="Flashing…" onAction={previewToolbarAlert}>Preview the alert</AsyncButton></div><p className="settings-help">The preview flashes the icon for five seconds — hover it while it flashes to see the episode list. The mark stays until you open the tracker.</p></section><section className="settings"><h2>Backup and restore</h2><p>Backups contain your library, progress, history, and settings. Provider images and episode metadata are refreshed after restore.</p><button onClick={download}>Export JSON backup</button><label className="file-button">Choose backup to restore<input hidden type="file" accept="application/json,.json" onChange={(event) => void inspectRestore(event)}/></label>
    <AutomaticBackups/>
